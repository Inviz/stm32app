...
/* 0x62XX0A: null */
#define transport_usart_set_phase(usart, value) device_set_property_numeric(usart->device, (uint32_t) value, sizeof(uint8_t), TRANSPORT_USART_PHASE)
#define transport_usart_get_phase(usart) *((uint8_t *) device_get_property_pointer(usart->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), TRANSPORT_USART_PHASE)
