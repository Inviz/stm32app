...
/* 0x62XX07: in us */
#define transport_modbus_set_phase(modbus, value) device_set_property_numeric(modbus->device, (uint32_t) value, sizeof(uint8_t), TRANSPORT_MODBUS_PHASE)
#define transport_modbus_get_phase(modbus) *((uint8_t *) device_get_property_pointer(modbus->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), TRANSPORT_MODBUS_PHASE)
