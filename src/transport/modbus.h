...
typedef struct transport_modbus_properties {
    ...
    uint16_t usart_index; // GPIO port for ready-to-send 
    uint8_t rts_port; // GPIO pin for ready-to-send 
    uint8_t rts_pin; // Leave 0 for master device 
    uint8_t slave_address; // Size of buffer to container usart message 
    uint16_t rx_buffer_size; // in us 
...
extern device_class_t transport_modbus_class;
...
typedef enum transport_modbus_properties_properties {
  TRANSPORT_MODBUS_USART_INDEX = 0x1,
  TRANSPORT_MODBUS_RTS_PORT = 0x2,
  TRANSPORT_MODBUS_RTS_PIN = 0x3,
  TRANSPORT_MODBUS_SLAVE_ADDRESS = 0x4,
  TRANSPORT_MODBUS_RX_BUFFER_SIZE = 0x5,
  TRANSPORT_MODBUS_TIMEOUT = 0x6,
  TRANSPORT_MODBUS_PHASE = 0x7
} transport_modbus_properties_properties_t;

/* 0x62XX07: undefined */
#define transport_modbus_set_phase(modbus, value) OD_set_u8(modbus->device->properties, TRANSPORT_MODBUS_PHASE, value, false)
#define transport_modbus_get_phase(modbus) *((uint8_t *) OD_getPtr(modbus->device->properties, TRANSPORT_MODBUS_PHASE, 0, NULL))
