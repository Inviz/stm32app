...
OD_ACCESSORS(transport, modbus, properties, usart_index, SUBIDX_MODBUS_USART_INDEX, uint16_t, u16) /* 0x62XX01: properties */
OD_ACCESSORS(transport, modbus, properties, rts_port, SUBIDX_MODBUS_RTS_PORT, uint8_t, u8) /* 0x62XX02: properties */
OD_ACCESSORS(transport, modbus, properties, rts_pin, SUBIDX_MODBUS_RTS_PIN, uint8_t, u8) /* 0x62XX03: properties */
OD_ACCESSORS(transport, modbus, properties, slave_address, SUBIDX_MODBUS_SLAVE_ADDRESS, uint8_t, u8) /* 0x62XX04: properties */
OD_ACCESSORS(transport, modbus, properties, rx_buffer_size, SUBIDX_MODBUS_RX_BUFFER_SIZE, uint16_t, u16) /* 0x62XX05: properties */
OD_ACCESSORS(transport, modbus, properties, timeout, SUBIDX_MODBUS_TIMEOUT, uint16_t, u16) /* 0x62XX06: properties */
OD_ACCESSORS(transport, modbus, properties, phase, SUBIDX_MODBUS_PHASE, uint8_t, u8) /* 0x62XX07: properties */
...
static app_signal_t modbus_validate(OD_entry_t *properties_entry) {
    transport_modbus_properties_t *properties = (transport_modbus_properties_t *)OD_getPtr(properties_entry, 0x00, 0, NULL);
    (void)properties;
    if (false) {
...
static app_signal_t modbus_phase_constructing(transport_modbus_t *modbus, device_t *device) {
    modbus->properties = (transport_modbus_properties_t *)OD_getPtr(device->properties, 0x00, 0, NULL);
    modbus->rx_buffer = malloc(modbus->properties->rx_buffer_size);
    return modbus->properties->disabled;
}
...
static app_signal_t modbus_phase_linking(transport_modbus_t *modbus) {
    return device_link(modbus->device, (void **)&modbus->usart, modbus->properties->usart_index, NULL) +
           device_link(modbus->device, (void **)&modbus->timer, modbus->properties->timer_index, NULL);
}
