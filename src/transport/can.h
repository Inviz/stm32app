...
/* 0x62XX0B: null */
#define transport_can_set_phase(can, value) actor_set_property_numeric(can->actor, (uint32_t) value, sizeof(uint8_t), TRANSPORT_CAN_PHASE)
#define transport_can_get_phase(can) *((uint8_t *) actor_get_property_pointer(can->actor, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), TRANSPORT_CAN_PHASE)
