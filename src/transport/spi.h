...
/* 0x62XX18: null */
#define transport_spi_set_phase(spi, value) device_set_property_numeric(spi->device, (uint32_t) value, sizeof(uint8_t), TRANSPORT_SPI_PHASE)
#define transport_spi_get_phase(spi) *((uint8_t *) device_get_property_pointer(spi->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), TRANSPORT_SPI_PHASE)
