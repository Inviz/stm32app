...
/* 0x61XX05: null */
#define module_timer_set_phase(timer, value) device_set_property_numeric(timer->device, (uint32_t) value, sizeof(uint8_t), MODULE_TIMER_PHASE)
#define module_timer_get_phase(timer) *((uint8_t *) device_get_property_pointer(timer->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), MODULE_TIMER_PHASE)
