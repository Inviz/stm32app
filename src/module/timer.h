...
typedef struct module_timer_properties {
    ...
    uint8_t prescaler; // How many subscriptions can timer hold at once 
    uint8_t initial_subscriptions_count; // Counter overflow value (usually max value, 16 or 32bit depending on counter) 
    uint32_t period; // How many times a second should timer fire 
    uint32_t frequency;
    uint8_t phase;
