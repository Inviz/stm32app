...
static int system_canopen_async_tick(system_canopen_t *canopen, void *argument, device_tick_t *tick, app_thread_t *thread) {
    ...
    if (us_until_next != (uint32_t)-1) {
        device_tick_delay(tick, thread, us_until_next / US_PER_TICK);
    }
    return 0;
}
...
static int system_canopen_input_tick(system_canopen_t *canopen, void *argument, device_tick_t *tick, app_thread_t *thread) {
    ...
    if (us_until_next != (uint32_t)-1) {
        device_tick_delay(tick, thread, us_until_next / 1000);
    }
    return 0;
}
...
static bool_t system_canopen_thread_wakeup(app_thread_t *thread) {
    app_event_t event = {
        ...
        .producer = thread->device->app->canopen->device,
        .consumer = thread->device->app->canopen->device
    };
    // canopen messages are safe to send in any order and it is desirable to handle them asap
    return app_thread_publish_to_front_from_isr(thread, &event);
}
...
device_callbacks_t system_canopen_callbacks = {
    ...
    .input_tick = (int (*)(void *, void *, device_tick_t *, app_thread_t *))system_canopen_input_tick,
    .async_tick = (int (*)(void *, void *, device_tick_t *, app_thread_t *))system_canopen_async_tick,
    .idle_tick = (int (*)(void *, void *, device_tick_t *, app_thread_t *))system_canopen_idle_tick,
