...
#define system_mcu_get_phase(mcu) *((uint8_t *) OD_getPtr(mcu->device->class->properties, SYSTEM_MCU_PHASE, 0, NULL))
#define system_mcu_set_cpu_temperature(mcu, value) OD_set_i16(mcu->device->properties, SYSTEM_MCU_CPU_TEMPERATURE, value, false)
#define system_mcu_get_cpu_temperature(mcu) *((int16_t *) OD_getPtr(mcu->device->class->properties, SYSTEM_MCU_CPU_TEMPERATURE, 0, NULL))
#define system_mcu_set_startup_time(mcu, value) OD_set_u32(mcu->device->properties, SYSTEM_MCU_STARTUP_TIME, value, false)
#define system_mcu_get_startup_time(mcu) *((uint32_t *) OD_getPtr(mcu->device->class->properties, SYSTEM_MCU_STARTUP_TIME, 0, NULL))
