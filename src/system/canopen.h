...
/* 0x60XX08: null */
#define system_canopen_set_phase(canopen, value) device_set_property_numeric(canopen->device, (uint32_t) value, sizeof(uint8_t), SYSTEM_CANOPEN_PHASE)
#define system_canopen_get_phase(canopen) *((uint8_t *) device_get_property_pointer(canopen->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), SYSTEM_CANOPEN_PHASE)
/* 0x60XX09: Node ID */
#define system_canopen_set_node_id(canopen, value) device_set_property_numeric(canopen->device, (uint32_t) value, sizeof(uint8_t), SYSTEM_CANOPEN_NODE_ID)
#define system_canopen_get_node_id(canopen) *((uint8_t *) device_get_property_pointer(canopen->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), SYSTEM_CANOPEN_NODE_ID)
/* 0x60XX0A: Negotiated bitrate */
#define system_canopen_set_bitrate(canopen, value) device_set_property_numeric(canopen->device, (uint32_t) value, sizeof(uint32_t), SYSTEM_CANOPEN_BITRATE)
#define system_canopen_get_bitrate(canopen) *((uint32_t *) device_get_property_pointer(canopen->device, &(uint8_t[sizeof(uint32_t)]{}), sizeof(uint32_t), SYSTEM_CANOPEN_BITRATE)
