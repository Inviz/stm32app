...
OD_ACCESSORS(screen, epaper, properties, spi_index, SUBIDX_EPAPER_SPI_INDEX, uint16_t, u16) /* 0x90XX01: properties */
OD_ACCESSORS(screen, epaper, properties, dc_port, SUBIDX_EPAPER_DC_PORT, uint8_t, u8) /* 0x90XX02: properties */
OD_ACCESSORS(screen, epaper, properties, dc_pin, SUBIDX_EPAPER_DC_PIN, uint8_t, u8) /* 0x90XX03: properties */
OD_ACCESSORS(screen, epaper, properties, cs_port, SUBIDX_EPAPER_CS_PORT, uint8_t, u8) /* 0x90XX04: properties */
OD_ACCESSORS(screen, epaper, properties, cs_pin, SUBIDX_EPAPER_CS_PIN, uint8_t, u8) /* 0x90XX05: properties */
OD_ACCESSORS(screen, epaper, properties, busy_port, SUBIDX_EPAPER_BUSY_PORT, uint8_t, u8) /* 0x90XX06: properties */
OD_ACCESSORS(screen, epaper, properties, busy_pin, SUBIDX_EPAPER_BUSY_PIN, uint8_t, u8) /* 0x90XX07: properties */
OD_ACCESSORS(screen, epaper, properties, reset_port, SUBIDX_EPAPER_RESET_PORT, uint8_t, u8) /* 0x90XX08: properties */
OD_ACCESSORS(screen, epaper, properties, reset_pin, SUBIDX_EPAPER_RESET_PIN, uint8_t, u8) /* 0x90XX09: properties */
OD_ACCESSORS(screen, epaper, properties, width, SUBIDX_EPAPER_WIDTH, uint16_t, u16) /* 0x90XX0a: properties */
OD_ACCESSORS(screen, epaper, properties, height, SUBIDX_EPAPER_HEIGHT, uint16_t, u16) /* 0x90XX0b: properties */
OD_ACCESSORS(screen, epaper, properties, mode, SUBIDX_EPAPER_MODE, uint16_t, u16) /* 0x90XX0c: properties */
OD_ACCESSORS(screen, epaper, properties, phase, SUBIDX_EPAPER_PHASE, uint8_t, u8) /* 0x90XX0d: properties */
/* End of autogenerated OD accessors */

/* Epaper needs DC, CS, BUSY, RESET pins set, as well as screen size */
static app_signal_t epaper_validate(OD_entry_t *properties_entry) {
    screen_epaper_properties_t *properties = (screen_epaper_properties_t *)OD_getPtr(properties_entry, 0x00, 0, NULL);
...
static app_signal_t epaper_phase_constructing(screen_epaper_t *epaper, device_t *device) {
    epaper->properties = (screen_epaper_properties_t *)OD_getPtr(device->properties, 0x00, 0, NULL);
    epaper->properties = (screen_epaper_properties_t *)OD_getPtr(device->properties, 0x00, 0, NULL);
    return 1; // epaper->properties->disabled;
}
...
static app_signal_t epaper_phase_starting(screen_epaper_t *epaper) {
    device_gpio_propertiesure_input("Busy", epaper->properties->busy_port, epaper->properties->busy_pin);
    device_gpio_propertiesure_output_with_value("Reset", epaper->properties->reset_port, epaper->properties->busy_pin, 0, 1);
    device_gpio_propertiesure_output_with_value("DC", epaper->properties->dc_port, epaper->properties->dc_pin, 0, 0);
    device_gpio_propertiesure_output_with_value("CS", epaper->properties->cs_port, epaper->properties->cs_pin, 0, 0);
...
device_methods_t screen_epaper_methods = {
    .validate = epaper_validate,
    .phase_constructing = (app_signal_t(*)(void *, device_t *))epaper_phase_constructing,
