...
/* 0x90XX0D: null */
#define screen_epaper_set_phase(epaper, value) device_set_property_numeric(epaper->device, (uint32_t) value, sizeof(uint8_t), SCREEN_EPAPER_PHASE)
#define screen_epaper_get_phase(epaper) *((uint8_t *) device_get_property_pointer(epaper->device, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), SCREEN_EPAPER_PHASE)
