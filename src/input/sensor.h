...
typedef struct input_sensor_properties {
    ...
    uint16_t disabled; // Values other than zero will prevent actor from initializing 
    ...
    uint8_t phase; // Current phase of a actor (one of values in DEVICE_PHASE enum) 
...
/* 0x80XX06: Current phase of a actor (one of values in DEVICE_PHASE enum) */
#define input_sensor_set_phase(sensor, value) actor_set_property_numeric(sensor->actor, (uint32_t) value, sizeof(uint8_t), INPUT_SENSOR_PHASE)
#define input_sensor_get_phase(sensor) *((uint8_t *) actor_get_property_pointer(sensor->actor, &(uint8_t[sizeof(uint8_t)]{}), sizeof(uint8_t), INPUT_SENSOR_PHASE)
