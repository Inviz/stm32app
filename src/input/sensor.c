...
OD_ACCESSORS(input, sensor, properties, disabled, SUBIDX_SENSOR_DISABLED, uint16_t, u16) /* 0x80XX01: properties */
OD_ACCESSORS(input, sensor, properties, port, SUBIDX_SENSOR_PORT, uint8_t, u8) /* 0x80XX02: properties */
OD_ACCESSORS(input, sensor, properties, pin, SUBIDX_SENSOR_PIN, uint8_t, u8) /* 0x80XX03: properties */
OD_ACCESSORS(input, sensor, properties, adc_index, SUBIDX_SENSOR_ADC_INDEX, uint16_t, u16) /* 0x80XX04: properties */
OD_ACCESSORS(input, sensor, properties, adc_channel, SUBIDX_SENSOR_ADC_CHANNEL, uint8_t, u8) /* 0x80XX05: properties */
OD_ACCESSORS(input, sensor, properties, phase, SUBIDX_SENSOR_PHASE, uint8_t, u8) /* 0x80XX06: properties */
...
static app_signal_t sensor_validate(OD_entry_t *properties_entry) {
    input_sensor_properties_t *properties = (input_sensor_properties_t *)OD_getPtr(properties_entry, 0x00, 0, NULL);
    (void)properties;
    if (false) {
...
static app_signal_t sensor_phase_constructing(input_sensor_t *sensor, device_t *device) {
    sensor->properties = (input_sensor_properties_t *)OD_getPtr(device->properties, 0x00, 0, NULL);
    return sensor->properties->disabled;
}
