...
static app_signal_t w25_tick_input(storage_w25_t *w25, app_event_t *event, device_tick_t *tick, app_thread_t *thread) {
    ...
    case APP_EVENT_INTROSPECTION:
        return device_event_handle_and_start_task(w25->device, event, &w25->task, w25->device->app->threads->async, w25_task_introspection);
    case APP_EVENT_WRITE:
        return device_event_handle_and_start_task(w25->device, event, &w25->task, w25->device->app->threads->async, w25_task_write);
    case APP_EVENT_READ:
        return device_event_handle_and_start_task(w25->device, event, &w25->task, w25->device->app->threads->async, w25_task_read);
    case APP_EVENT_LOCK:
        return device_event_handle_and_start_task(w25->device, event, &w25->task, w25->device->app->threads->async, w25_task_lock);
    case APP_EVENT_UNLOCK:
        return device_event_handle_and_start_task(w25->device, event, &w25->task, w25->device->app->threads->async, w25_task_unlock);
