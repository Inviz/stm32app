...
/* A generic RTOS task function that handles all typical configurations of threads. It supports following features or combinations of them:
  ...
  An app has at least five of these threads with different priorities, so devices can declare their work with a specific priority. In
  addition devices can also create their own custom threads with own queues, leveraging all the dispatching and queue-management mechanisms.
 */
